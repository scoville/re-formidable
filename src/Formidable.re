...
module States = {
  module Field = {
    ...
    type t('values, 'error) = {
      ...
      validate:
        (option(Validations.Strategy.t), 'values) => Status.t('error),
    };
  ...
  let getErrors = fields =>
    fields->Map.String.reduce([], (acc, _, field) =>
      field
      ->Field.getErrors
      ->Option.mapWithDefault(acc, errors => acc @ errors)
    );
...
module Hook = {
  type t('values, 'error) = {
    ...
    submit:
      unit => Js.Promise.t(Map.String.t(States.Field.t('values, 'error))),
  };
};
...
module type Form = {
  ...
  module Provider: {
    [@bs.obj]
    external makeProps:
      (~children: React.element, ~key: string=?, unit) =>
...
module Make =
       (
         ...
         Error: Type,
         Handlers:
           Handlers with type values = Values.t and type error = Error.t,
       )
  ...
  let use = () => {
    ...
    let submit = () => {
      let fields =
        fields->Map.String.map(({States.Field.validate} as field) =>
          {...field, status: validate(None, values)}
        );

      switch (
        Handlers.onSubmit,
        Handlers.onSubmitError,
        States.getErrors(fields),
      ) {
    ...
    let make = props => {
      ...
      let modifiers = {
        Context.removeField: name =>
          setState(state =>
            {...state, fields: state.fields->Map.String.remove(name)}
          ),
        setIsDisabled: isDisabled =>
          setState(state => {...state, isDisabled}),
        setField: (name, field) =>
          setState(state =>
            {...state, fields: state.fields->Map.String.set(name, field)}
          ),
        setFields: fields => setState(state => {...state, fields}),
        setValues: values => setState(state => {...state, values}),
        updateField: (name, f) =>
          setState(state =>
            {
              ...state,
      ...
      React.createElement(
        provider,
        {"children": props##children, "value": (state, modifiers)},
      );
    };
  };
  ...
  module Field = {
    ...
    let make = props => {
      ...
      let validationNames =
        props##validations
        ->Option.mapWithDefault([], validations =>
      ...
      let validate =
        React.useCallback1(
          (validationContext, values) =>
            props##validations
            ->Option.mapWithDefault(`valid, validations =>
                switch (
                  validations->List.keepMap(((strategy, (_, validation))) =>
                    if (Validations.shouldValidate(
      ...
      let validateAndUpdate = (validationContext, values) =>
        modifiers.updateField(props##name, field =>
          {...field, status: validate(validationContext, values)}
        );
      ...
      let setStatus = status =>
        modifiers.updateField(props##name, field => {...field, status});
      ...
      children({
        Props.Field.isDisabled:
          isDisabled || props##disable->Option.getWithDefault(false),
        isFocused,
        label: props##label,
        name: props##name,
        onBlur,
  ...
  let make = props => {
    let (_, {Context.setIsDisabled}) = React.useContext(context);
    ...
    ReactDOMRe.createElement(
      ...
      ~props=
        Obj.magic({
          ...
          "action": props##action,
          "_method": props##method->Option.map(FormMethod.toString),
          "onSubmit":
            Events.handle'(
...
type t('values, 'error) = (module Form with
                              type error = 'error and type values = 'values);
...
let use = (~values, ~error, ~onSubmit=?, ~onSubmitError=?, ()) =>
  React.useMemo0(() =>
    make(~values, ~error, ~onSubmit?, ~onSubmitError?, ())
  );
